...
class Vector:
	...
	func draw(node, camera):
		var start = camera.unproject_position(object.get_node("Proto_Drone_Mesh/P1").global_transform.origin)
		var end = camera.unproject_position(object.get_node("Proto_Drone_Mesh/P1").global_transform.origin + object.transform.basis.y * (-object.roll) * scale)
